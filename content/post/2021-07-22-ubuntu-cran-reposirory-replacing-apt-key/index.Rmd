...
As the warning suggests, keyring files should be located in the `/etc/apt/trusted.gpg.d/` directory.  First step is to download my .asc key file, which is an armored, ASCII version of my public key file.
...
Once you have verified the key, you can copy the .asc file to `/etc/apt/trusted.gpg.d/`.  It is also possible to download the file and copy it to the proper direcotry with the following command:
